...
fn emit_statement<'a>(w: &mut Write, statement: &Statement<'a>) -> fmt::Result {
    match statement {
        &Statement::Assignment{id: _, ref value} => emit_assignment(w, value)?,
        &Statement::LocalAssignment{id: _, ref value} => emit_local_assignment(w, value)?,
        &Statement::FunctionCall{id: _, ref value} => emit_function_call(w, value)?,
        &Statement::NumericFor{id: _, ref value} => emit_numeric_for(w, value)?,
        &Statement::GenericFor{id: _, ref value} => emit_generic_for(w, value)?,
        &Statement::IfStatement{id: _, ref value} => emit_if_statement(w, value)?,
        &Statement::WhileLoop{id: _, ref value} => emit_while_loop(w, value)?,
        &Statement::RepeatLoop{id: _, ref value} => emit_repeat_loop(w, value)?,
        &Statement::FunctionDeclaration{id: _, ref value} => emit_function_declaration(w, value)?,
    }
